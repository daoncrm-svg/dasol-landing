...
function sanitizeAdData(adData) {
  ...
  return {
    ...
    fbclid: sanitizeOptionalString(source.fbclid, 120),
...
function deriveCampaignAttribution(adData) {
  ...
  const source = String(adData?.utm_source || '').toLowerCase();
  const referrer = String(adData?.referrer || '').toLowerCase();
  ...
  const isMetaAds =
    Boolean(adData?.fbclid) ||
    ['meta', 'facebook', 'instagram', 'ig', 'fb'].includes(source) ||
  ...
  const sourcePlatformLabel = isGoogleAds
    ? 'GOOGLE'
    : isMetaAds
      ? 'META'
      : null;
...
function buildLeadPayload(submission) {
  const sourceLabel = submission.attribution?.sourcePlatformLabel
    ? `${submission.attribution.sourcePlatformLabel} | ${submission.source}`
    : submission.source;
